...
from config.basemodel import Error, TaiwanHotDamage, TaiwanUVIndex
    ...
    while county_hot_damage_list == []:
        ...
        for county_data in counties_data:
            ...
            for town_data in county_data["Location"]:
                ...
                town_hot_damage = {"town": town_name, "data": hot_damage_list}
                town_hot_damage_list.append(town_hot_damage)
            county_hot_damage = {"county": county_name, "data": town_hot_damage_list}
...
def fetch_uv():
    ...
    while county_UV_Index_list == []:
        ...
        for location in data["records"]["weatherElement"]["location"]:
            ...
            if (
                county not in max_uv_by_county
                or uv_index > max_uv_by_county[county]["UVIndex"]
            ):
                max_uv_by_county[county] = {"county": county, "UVIndex": uv_index}
...
@router.get(
    "/api/hotdamage",
    responses={200: {"model": TaiwanHotDamage, "description": "資料取得成功"}},
    response_class=JSONResponse,
    summary="取得各縣市鄉鎮五天內當日最大熱傷害指數與傷害警示",
)
...
@router.get(
    "/api/UV",
    responses={200: {"model": TaiwanUVIndex, "description": "資料取得成功"}},
    response_class=JSONResponse,
    summary="取得當日各縣市紫外線指數",
)
