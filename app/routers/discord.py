...
from config.basemodel import Error
...
async def discord_bot(
    request: Request,
    weather: Annotated[
        dict, Depends(lambda county_name="臺北市": get_weather_by_county(county_name))
    ],
    hot: Annotated[dict, Depends(get_hot_damage)],
    uv: Annotated[dict, Depends(get_uv)],
):
        ...
        "embeds": [
            {
                ...
                "timestamp": timestamp,
                "color": 1572632,
                "footer": {"text": "by 聖鎧、書硯、宜群、陳昊"},
                "thumbnail": {"url": thumbnail_url, "height": 1000, "width": 1400},
                "author": {"name": "下禮拜三就是睡衣派對!", "icon_url": avatar_url},
                "fields": [
                    {"name": "天  氣🌤️", "value": api_array_list[1], "inline": True},
                    {
                        "name": "降  雨🌧️",
                        "value": f"{api_array_list[5]}%",
                        "inline": True,
                    },
                    {
                        "name": "氣  溫🌡️",
                        "value": f"{api_array_list[3]}~{api_array_list[2]}℃",
                        "inline": True,
                    },
                ...
                "image": {"url": image_url, "height": 375, "width": 1250},
            }
        ],
    }
